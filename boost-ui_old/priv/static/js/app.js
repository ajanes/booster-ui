...
var channel = exports.channel = {
  on_update: function on_update(f) {
    var channel = _socket2.default.channel("update:graph");
    channel.on("update_message", function (payload) {
      return f(payload);
    });
  }
};
...
var channel = socket.channel("update:graph");

channel.join().receive("ok", function (resp) {
   console.log("Joined the update channel successfully.", resp);
}).receive("error", function (resp) {
   console.log("Unable to join the update channel.", resp);
});
